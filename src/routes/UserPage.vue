...
watch(username, async () => {
  try {
    userData.value = await api.getUser(username.value);
  } catch (e) {
  ...
  for (let i = 0; i < (await userData.value.projects).length; i++) {
    newProjects.push(api.EMPTY_PROJECT);
    newProjects[newProjects.length - 1]._loading = true;
  }

  for (let i = 0; i < (await userData.value.collaborations).length; i++) {
